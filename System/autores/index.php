<?php
    ...
    $COUNT_Results = Autor::SELECT_COUNT($db);
    $Objects = Autor::SELECT_Autores($db);
    $paginacao = new Paginacao($COUNT_Results, 5, $paginaAtual);

    $linksPaginas = $paginacao->getLinkPages();
    
    $Autores = $paginacao->getResultsByPage($Objects);
?>
<div class="bg-dark p-3" style="font-size: 13pt">
        <?php if($Autores == false) { ?>
            <div class="text-center text-light">Nenhum autor encontrado - <a href="cadastrar.php">Cadastrar</a></div>
        <?php die();
            } echo $linksPaginas; ?>
    ...
    <table class="table text-light align-middle">
        ...
        <tbody  class="border">
            <?php foreach($Autores as $Autor) { ?>
                <tr>
                    <td class="px-5"><?php echo $Autor->getID() ?></td>
                    <td><?php echo "{$Autor->getNome()} {$Autor->getSobrenome()}" ?></td>
                    
                    <td>
                        <a class="px-3 text-info" href="../?By=autor&busca=<?php echo "{$Autor->getNome()} {$Autor->getSobrenome()}" ?>">Pesquisar Livros</a>
                        <a href="editar.php?id=<?php echo $Autor->getID() ?>">
                            <button type="button" class="btn btn-primary">Editar</button>
                        </a>
                        <a href="excluir.php?id=<?php echo $Autor->getID() ?>">
